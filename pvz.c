...
against libsubstrate.dylib. Also, libstubhook mods obviously need
to have libstubhook as a depend in the control file, so don't
forget that.
*/

//Dnclude stubhook header to be able to use libstubhook
#include <stubhook.h>
...
//Define function pointers to the internal PvZ methods we want to
//call later.
...
//Hooks the method that is called to get the Plant cost. We'll
//set the return value to 0, which means the Plant cost is 0
SHHook(int, Plant$$getCost, void *self, SeedType *seedType, SeedType *seedTypeX) {
	return 0;
}

//The two hooks above check if the plant is sleeping or a night
//plant, both set to false effects that you can plant night
//plants at day and they still do their job and do not sleep.
...
//This hook instantly kills the zombie when it starts eating. You
//can either let zombies die, freeze them, slow them
SHHook(void, Zombie$$startEating, void *self) {
	Zombie$$ApplyBurn(self);
	return _Zombie$$startEating(self);
}

//This hook calls the collect method when a sun or a coin gets
//dropped. this effects an auto-collect hook
...
//The refresh time is how long a plant needs till you can plant
//it again. settingg it to 0 effects instant planting
...
//Does not matter if you have enough money, you can buy items in
//store, because we set CanAffordItem to always true.
...
//This calls the addsunmoney when it normally subtract the money,
//so you get the money you spent for plants
SHHook(void, Board$$TakeSunMoney, void *self, int sunMoney) {
	Board$$AddSunMoney(self,sunMoney);
}

//Empty hook means, that the method does not do anything. so the
//fog will not be drawn
SHHook(void, Board$$DrawFog, void *self, void *graphics) {
	
}

//Same here, neither craters nor ladders will be added to the
//board
...
SHConstructor {
	/*
	If the app version is correct (1.9.6 = CFBundleShortVersion,
	4111.0.0 = Version) it will actually stub the function
	pointer.
	...
	*/
	...
	// Now that we set the addresses, actually apply the hooks!
